    ...
    <main className="min-h-screen bg-white">
      ...
      <section id="categorias" className="max-w-7xl mx-auto px-6 py-20">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {categorias.map((cat) => (
            <Link key={cat.slug} href={`/categorias/${cat.slug}`} className="group relative h-64 overflow-hidden rounded-sm">
      ...
      {/* El resto del catálogo general se queda abajo igual que antes */}
      <section id="productos" className="max-w-7xl mx-auto px-6 py-20 border-t border-slate-100">
        <h2 className="text-xs font-black uppercase tracking-[0.4em] text-slate-400 mb-12 text-center">La Selección Completa</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          {productos.map((p, index) => (
            <article key={p.id}>
              <Link href={`/productos/${p.id}`} className="block aspect-[4/5] bg-[#f5f5f5] overflow-hidden">
